The scripts in this git repository contain the main algorithms for computing Novel PhyloCSF Regions, as defined in "Whole-genome PhyloCSF reveals 144 high-confidence human protein-coding genes, elucidating many disease variants", by Jonathan M. Mudge, Irwin Jungreis, et al.
...
Summary of the workflow for creating Novel PhyloCSF Regions:
...
Step 4. Call SVMsForNovelPhyloCSFRegions.classify_regions, supplying the path to the PhyloCSF Region files produced in step 3, and user-supplied bed-format files containing the coding and pseudogene annotations for the genome. This will intersect the PhyloCSF Regions with the annotations, produce an initial spreadsheet, and produce the input file for running PhyloCSF in step 5.
...
Step 6. Call SVMsForNovelPhyloCSFRegions.do_other_steps. This trains SVMs to distinguish regions most likely to be coding; eliminates regions overlapping annotations, short regions, and regions more likely to be antisense; and sorts the remaining regions, "Novel PhyloCSF Regions", according to how likely they are to be true coding regions. The result is a bed-format file, PhyloCSFNovel.bed, containing the sorted list of Novel PhyloCSF Regions, and a spreadsheet, Regions.04.txt, containing the associated scores.
